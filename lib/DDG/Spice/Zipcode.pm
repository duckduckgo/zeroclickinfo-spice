...
triggers query_lc => qr/(zip\s*(code)?|post(al)?\s*(code)?)?\s*(([a-z0-9]{3}\s?[a-z0-9]{3}|([0-9]{5}(\-[0-9]{4})?))\s*(\(([a-z]{2})\)|(\w+))?)/;
...
handle query_lc => sub {
	...
	if (m/((?:[a-z0-9]{3}\s?[a-z0-9]{3})|(?:[0-9]{5}(?:\-[0-9]{4})?))/){
		$code = $1;
		$code =~ s/\s*//g;
