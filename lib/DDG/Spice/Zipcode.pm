...
handle query_lc => sub {
	...
	# Regexs to disqualify remainder
	unless (m/\d/ or m/^\s*[a-z]{2}\s*[a-z]{2}\s*$/) {
		warn "NO DIGITS";
		return;
	}
	...
	if (m/\d{16,}$/) {
		warn "TOO MANY DIGITS";
		return;
	}

	# Check for known incorrect triggers
	if (m/\b
					(
							4 \s* chan
						|	\d+ \s* (st|nd|rd|th)
	...
	if (m/ \b
			...
			/x) {
		warn "MATCHED EDGE CASE: |$1|";
	}

	elsif (m/ \b ( (?: \d{1,4} | [a-z]{2} ) \d\-\d\d \d{1,4} ) \b /x) {
		warn "MATCHED NUM WITH DASH: |$1|";
		$code = $1;
	}
						...
						(
				 		...
				 		\b
				 /x) {
		warn "MATCHED ALPHA-NUM WITH SPACE: |$1|";
		$code = $1;
	}
						...
						\b
				/x) {
 	  warn "MATCHED NO SPACE: |$1|";
 	  $code = $1;
	}

  elsif (m/ \b
  			( \d{2,7} ) \b /x) {
		warn "MATCHED STRAIGHT NUMS: |$1|";
		$code = $1;
	}

	else {
		warn "---NO POSTAL CODE MATCH!---";
		warn "\n\n";
		return;
	};
	...
	if (m/[a-z]{5,}$/ and $_ !~ m/(\d\d [a-z\d\-]+ \d\d)/x ) {
		warn "TOO MANY LETTERS";
		return;
	}

	return uri_escape(uc($code)), uc $countries{$country} unless !defined $countries{$country};
