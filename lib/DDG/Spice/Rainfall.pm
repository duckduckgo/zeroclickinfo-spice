...
handle remainder => sub {
    ...
    #loop to check vaild reporting dates against current date
    foreach my $date (@reportingDates){
          if($curYear > $date || $curYear == $date) {
          $vaildDate = join(':', $date,$date);
         }
    }    

    if (defined $country and defined $vaildDate) {
        return uc $countries{$country}, $vaildDate;
    }
    return;
};
1;
