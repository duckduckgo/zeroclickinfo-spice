function ddg_spice_lastfm_album(api_result) {
    "use strict";
    var skip = [ "album", "albums", "records", "cd", "cds" ];
    ...
    if (DDG.isRelevant(api_result.album.name, skip) && DDG.isRelevant(api_result.album.artist, skip) && +api_result.album.playcount > 1e3) {
        Spice.add({
            data: api_result,
            ...
            $("#expand").click(function() {
                DDG.toggle("all", 1), DDG.toggle("some", -1), DDG.toggle("expand", -1);
            });
        });
    }
}
...
Handlebars.registerHelper("cleanDate", function(date, options) {
    ...
    if (// Make sure it's not a string filled with whitespace.
    date = date.replace(/^\s+|\s+$/g, ""), // Let's trim the string.
    date = date.substr(0, date.lastIndexOf(",")), date) {
        options.fn({
            date: date
        });
    }
...
Handlebars.registerHelper("checkWiki", function(wiki, options) {
    ...
    if (wiki && wiki.summary) {
        if (// Make sure summary isn't full of white space.
        summary = wiki.summary.replace(/^\s*$/g, ""), "" !== summary) {
...
Handlebars.registerHelper("list", function(items, options) {
    "use strict";
    var tracks = [];
    for (var i = 0; i < items.length && 5 > i; i += 1) {
    ...
    return first = first.join(", "), first + ", and " + tracks[tracks.length - 1] + ".";
