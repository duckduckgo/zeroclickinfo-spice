!function(env) {
    env.ddg_spice_betterific = function(api_result) {
        ...
        var kinds = [ "betterifs", "tags", "users" ], s = 0, kind;
        for (var i = kinds.length - 1; i >= 0; i--) {
            if (kind = kinds[i], !api_result[kind] || !api_result[kind][kind]) {
                return Spice.failed("betterific");
            }
        ...
        var obj_length;
        for (var i = kinds.length - 1; i >= 0; i--) {
            if (kind = kinds[i], (obj_length = api_result[kind][kind].length) > 0) {
...
}(this), /* Adapted from Betterific's lib/ruby_extensions.rb
 *   def dashed
