!function(env) {
    ...
    env.ddg_spice_coupon_mountain = function(api_result) {
        ...
        Spice.add({
            ...
            onShow: highlightCode
        });
    };
    function getExpiry(dateString) {
        // 3333-03-03 means coupon has no expiry date
        if (!dateString || "3333-03-03" === dateString) {
            return null;
        }
    }
    Spice.registerHelper("CouponMountain_dateString", function(string) {
        var months = [ "Jan.", "Feb.", "Mar.", "Apr.", "May", "Jun.", "Jul.", "Aug.", "Sep.", "Oct.", "Nov.", "Dec." ], date = DDG.getDateFromString(dateString);
