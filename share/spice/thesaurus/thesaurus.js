!function(env) {
    env.ddg_spice_thesaurus = function(api_result) {
        ...
        var script = $('[src*="/js/spice/thesaurus/"]')[0], source = $(script).attr("src"), match = source.match(/\/js\/spice\/thesaurus\/([^\/]+)\/([^\/]+)/), query = match[1], mode = match[2];
        var shorthand = {
            synonyms: "syn",
            synonym: "syn",
            antonyms: "ant",
        ...
        api_result.mode = shorthand[mode], // Create the plugin.
        Spice.add({
            id: "thesaurus",
            name: "Thesaurus",
            data: api_result,
