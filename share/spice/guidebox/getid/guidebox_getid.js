function ddg_spice_guidebox_getid(api_result) {
    ...
    var SKIP_ARRAY = [ "online", "tv", "episode", "episodes", "free", "guidebox", "watch", "full" ], results = api_result.results.result, relevant;
    // Exit if we didn't find anything relevant.
    if (// Check which show is relevant to our query.
    ...
    var script = $('[src*="/js/spice/guidebox/getid/"]')[0], source = decodeURIComponent($(script).attr("src")), matched = source.match(/\/js\/spice\/guidebox\/getid\/([a-zA-Z0-9\s]+)/), query = matched[1];
    var metadata = {
    ...
    ddg_spice_guidebox_getid.metadata = metadata, $.getScript("/js/spice/guidebox/lastshows/series/" + relevant.id);
}
...
Handlebars.registerHelper("checkSeason", function(season_number, episode_number, options) {
    ...
    "use strict";
    var aired = DDG.getDateFromString(first_aired), days = [ "Sunday", "Monday", "Tuesday", "Wednesday", "Thursday", "Friday", "Saturday" ], months = [ "January", "February", "March", "April", "May", "June", "July", "August", "September", "October", "November", "December" ];
