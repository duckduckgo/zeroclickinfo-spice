function ddg_spice_forecast(r) {
  ...
  if(!current_location && !(/\d/).test(query) && !relevant_location) {
    ...
    units = r.flags && r.flags.units || "us", unit_labels = {
        us: {
            speed: "mph",
            temperature: "F"
        },
    ...
    var available_skycon_icons = [ "rain", "snow", "sleet", "wind", "fog", "cloudy", "partly_cloudy_day", "partly_cloudy_night", "clear_day", "clear_night" ];
    var skycon_type = function(icon) {
        if ("hail" === icon) {
            icon = "sleet";
        }
        if (icon = icon.replace(/-/g, "_"), -1 === $.inArray(icon, available_skycon_icons)) {
            icon = "cloudy";
        }
        return icon;
    };
    ...
    var build_currently = function(f) {
        var now = new Date().getTime() / 1e3, hourly = f.hourly.data, current_summary = f.currently.summary, speed_units = unit_labels[units].speed, feel_str, currentObj = {};
        // If the next-hour summary is interesting enough (and we're not on mobile), use that instead
        if (currentObj.isCurrent = 1, !is_mobile && f.minutely && !f.minutely.summary.match(/ for the hour\.$/)) {
    ...
    */
        ...
        if (currentObj.temp = temp_str, currentObj.feelslike = feel_str, current_summary.length > 45) {
            currentObj.summaryClass = "fe_small";
        } else {
            currentObj.summaryClass = "";
        }
        if (currentObj.summary = current_summary, f.currently.windSpeed) {
            var wind_speed = Math.round(f.currently.windSpeed);
            if (0 != wind_speed && f.currently.windBearing) {
        ...
        return currentObj.icon = get_skycon(skycon_type(f.currently.icon)), currentObj;
    };
    // Build the list of days
    var build_daily = function(f) {
        var dailyObj = [], day_strs = [], today = new Date(), today_i = today.getDay(), month_i = today.getMonth(), date_i = today.getDate(), days = f.daily.data, num_days = Math.max(6, days.length), day, temp_span, max_temp_height = 65, high_temp = -1/0, low_temp = 1/0;
        if (!is_mobile) {
        ...
        for (var i = 0; num_days > i; i++) {
            if (day = days[i], day.temperatureMax > high_temp) {
        ...
        // store daily values
        for (var i = 0, tmp_date; num_days > i; i++) {
            (function(i) {
                dailyObj[i] = days[i], day = days[i], tmp_date = new Date(), tmp_date.setDate(date_i + i), 
                dailyObj[i].date = tmp_date.toDateString().substr(4, 6), dailyObj[i].day = 0 == i ? "Today" : day_strs[(today_i + i) % 7], 
                dailyObj[i].highTemp = Math.round(day.temperatureMax) + "&deg;", dailyObj[i].lowTemp = Math.round(day.temperatureMin) + "&deg;", 
                dailyObj[i].icon = get_skycon(skycon_type(days[i].icon)), dailyObj[i].tempBar = {
    ...
    var build_alerts = function(f) {
        ...
        for (var i = 0; i < f.alerts.length; i++) {
            if (f.alerts[i].title.match(/Special Weather Statement/i) || f.alerts[i].title.match(/Advisory/i) || f.alerts[i].title.match(/Statement/i)) {
                continue;
            }
    ...
    function build_temp_switch(current_unit) {
        if ("F" === current_unit) {
            $("#fe_celsius").addClass("fe_gray").removeClass("fe_selected"), $("#fe_fahrenheit").removeClass("fe_gray").addClass("fe_selected");
        } else {
            if ("C" === current_unit) {
                $("#fe_fahrenheit").addClass("fe_gray").removeClass("fe_selected"), $("#fe_celsius").removeClass("fe_gray").addClass("fe_selected");
            }
        }
    }
    // if there's alerts add them to the end:
    if (// Go!
    weatherData.current = build_currently(r), weatherData.alerts = build_alerts(r), 
    weatherData.daily = build_daily(r), weatherData.activeUnit = unit_labels[units].temperature, 
    weatherData.city = r.flags["ddg-location"], // build the header text:
