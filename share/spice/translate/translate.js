function get_params(name, api_result) {
    ...
    if (match.length) {
        match.shift();
        //match array[0] is always full phrase
        // check if we're detecting a language
        var flag = "detect" === name, script, source, lang_match, to_lang, from_lang;
        // grab language to translate from/to
        // if not in query
        if (!match[1]) {
            if (flag) {
                script = $("[src*='js/spice/translate/detect']")[0], source = $(script).attr("src"), 
                lang_match = source.match(/([a-z]{2})$/), to_lang = lang_match[0];
            } else {
                script = $("[src*='js/spice/translate/from_to']")[0], source = $(script).attr("src"), 
                lang_match = source.match(/from_to(?:_phrase)?\/([a-z]{2})/), from_lang = lang_match[1];
            }
        }
        var out = {};
        if (out.phrase = match[0], flag) {
            out.from = api_result.data.detections[0].language, out.to = match[1] || to_lang;
        } else {
            out.from = match[1] || from_lang, out.to = match[2];
        }
        return out.short_from = shorten_lang(out.from, flag), out.short_to = shorten_lang(out.to, flag), 
        out.from_to = out.short_from + out.short_to, out.phrase_len = match[0].split(" ").length, 
        out;
    }
    return void 0;
}
...
function ddg_spice_translate_from_to(api_result) {
    ...
    var params = get_params("from_to"), endpoint;
    Spice.add({
        data: api_result,
