function ddg_spice_translate_detect(api_result) {
    if (!api_result.data.detections.length) {
        return;
    }
    var params = get_params("detect", api_result);
    // translate single word
    if (1 === params.phrase_len && params.from && params.to) {
        // Wordreference API only supports translations
        // to or from English...
        if (-1 === params.from_to.indexOf("en")) {
            endpoint = "/js/spice/translate/from_to_phrase/", nrj(endpoint + params.short_from + "/" + params.short_to + "/" + params.phrase);
        } else {
            endpoint = "/js/spice/translate/from_to/", nrj(endpoint + params.from_to + "/" + params.phrase);
        }
    }
}
...
Handlebars.registerHelper("getTerms", function(input, options) {
    ...
    return $.map(input, function(value, key) {
        $.map(value, function(value2, key2) {
            if ("OriginalTerm" !== key2 && value2.term && !unique[value2.term]) {
