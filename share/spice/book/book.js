function ddg_spice_book(api_result) {
    ...
    if (!api_result || 0 == api_result.books.length) {
    ...
    for (var i = 0; i < api_result.books.length; i++) {
        ...
        if ((api_result.books[i].review_count || api_result.books[i].unrated_review_count) && (DDG.stringsRelevant(title + " " + sub_title, decoded_query, [], 3, !0) || DDG.stringsRelevant(title, decoded_query, [], 3, !0) || DDG.stringsRelevant(title + " " + author, decoded_query, [], 3, !0) || DDG.stringsRelevant(title + " " + sub_title + " " + author, decoded_query, [], 3, !0))) {
    ...
    if (0 === data.review_count) {
        data.critic_reviews = data.unrated_critic_reviews, data.review_count = data.unrated_review_count;
    }
    ...
    data.critic_reviews = critic_reviews, // Pick a random critic review out of all the reviews returned
