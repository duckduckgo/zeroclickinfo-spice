function ddg_spice_songkick_getartistid(api_result) {
    ...
    if (0 == api_result.resultsPage.results.totalEntries || 0 == api_result.resultsPage.results.length) {
        return;
    }
    if (!api_result.resultsPage.results.artist || 0 == api_result.resultsPage.results.artist.length) {
        return;
    }
    ...
    ddg_spice_songkick_getartistid.metadata = metadata, $.getScript("/js/spice/songkick/artists/" + artist_id);
}
...
function ddg_spice_songkick_artists(artists_data) {
    ...
    if (0 == artists_data.resultsPage.results.totalEntries || 0 == artists_data.resultsPage.results.length) {
        return;
    }
    if (!artists_data.resultsPage.results.artist || 0 == artists_data.resultsPage.results.artist.length) {
        return;
    }
    Spice.add({
        data: artists_data,
