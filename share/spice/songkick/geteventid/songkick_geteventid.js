!function(env) {
    ...
    env.ddg_spice_songkick_geteventid = function(api_result) {
        var totalEntries = DDG.getProperty(api_result, "resultsPage.totalEntries"), location = DDG.getProperty(api_result, "resultsPage.results.location"), considered_location = DDG.getProperty(api_result, "resultsPage.results.location.0");
        ...
        Spice.add({
            ...
            normalize: function(item) {
                if (item.performance.length > 0 && "Unknown venue" !== item.venue.displayName) {
                    return {
    ...
    function twelve_hour(t) {
        ...
        var time = t.split(/:/), hours = +time[0], minutes = +time[1];
