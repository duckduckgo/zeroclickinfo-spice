!function(env) {
    ...
    env.ddg_spice_quixey = function(api_result) {
        ...
        var categories = [ "action", "adventure", "arcade", "board", "business", "casino", // "calculator",
        "design", "developer tools", "dice", "education", "educational", "entertainment", "family", "finance", "fitness", "graphics and design", "graphics", "health and fitness", "health", "kids", "lifestyle", "map", "medical", "music", "navigation", "networking", "news", "photography", "productivity", "puzzle", "racing", "role playing", "simulation", "social networking", "social", "sports", "strategy", "travel", "trivia", "utilities", "video", "weather" ], // force startend to prevent categories
        // from doing silly stuff on queries like
        // 'yahoo news digest'
        category_alt_start = categories.join("|^"), category_alt_end = categories.join("$|"), category_trigger_regexp = new RegExp("^" + category_alt_start + "|" + category_alt_end + "$", "i"), category_match_regexp = new RegExp(qLower.match(category_trigger_regexp), "i");
        Spice.add({
            id: "apps",
            name: "Apps",
            ...
            normalize: function(item) {
                ...
                if (screenshot = DDG.getProperty(item, "editions.0.screenshots.0.image_url"), !screenshot) {
                    return null;
                }
                if (item.name && item.name.toLowerCase() === qLower) {
                    item.exactMatch = !0;
                } else {
                    if (item.developer && item.developer.name && item.developer.name.toLowerCase() === qLower) {
                        item.boost = !0;
                    }
                }
                return {
            ...
            relevancy: {
                ...
                skip_words: [ "android", "app", "apple app store", "apple app", "application", "applications", "apps", "best", "blackberry", "download", "downloaded", "droid", "enhanced", "free", "fully featured", "google play store", "google play", "ios", "ipad", "iphone", "ipod touch", "ipod", "most", "playbook", "popular", "release data", "release", "sale", "search", "windows mobile", "windows phone 8", "windows phone" ],
                category: [ {
                    required: "icon_url"
                }, {
                    key: "short_desc"
            ...
            sort_fields: {
                ...
                function(a, b) {
                    var rank = function(r, count) {
                        ...
                        // rating_count is sometimes null
    ...
    var make_icon_url = function(item) {
        var domain = quixey_image_domain, icon_url = item.icon_url;
        if (!icon_url) {
    ...
    var pricerange = function(item) {
        ...
        for (var i in item.editions) {
            if (tmp = item.editions[i].cents, low > tmp) {
                low = tmp;
            }
            if (tmp > high) {
                high = tmp;
        ...
        if (lowp = qprice(low), high > low) {
            highp = qprice(high), range = lowp + " - " + highp, item.hasPricerange = !0;
        } else {
            range = lowp;
        }
        return range;
    ...
    Handlebars.registerHelper("Quixey_platform_name", function() {
        ...
        if (name = platforms[0].name, platforms.length > 1) {
