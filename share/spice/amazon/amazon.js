function ddg_spice_amazon(api_result) {
    "use strict";
    if (!api_result || !api_result.results || !api_result.results.length || 0 == api_result.results.length) {
        return Spice.failed("products");
    }
    var items = api_result.results, loadedRatingsData = !1;
    Spice.add({
        id: "products",
        name: "Products",
        data: items,
        ...
        onShow: function() {
            ...
            var onGotRatingsData = function(r, s, x) {
                ...
                if (this.reviewCount = r.reviews, this.$html) {
                    var $ratingsWrapper = this.$html.find(".tile__rating");
                    if ($ratingsWrapper && $ratingsWrapper.length) {
                        $ratingsWrapper.html(Handlebars.helpers.starsAndReviews(this.rating, this.reviewCount, this.url_review, !0));
                    }
                }
            };
            for (var i = 0; i < items.length; i++) {
                var item = items[i], arg = item.rating, url = "/m.js?r=";
                arg = arg.replace(/(?:.com.au|.com.br|.cn|.fr|.de|.in|.it|.co.jp|.mx|.es|.co.uk|.com|.ca?)/i, ""), 
                arg = arg.replace("http://www.amazon/reviews/iframe?", ""), $.getJSON(url + encodeURIComponent(arg), onGotRatingsData.bind(item));
            }
