!function(env) {
    ...
    env.ddg_spice_movie = function(api_result) {
        ...
        // Get original query.
        var script = $('[src*="/js/spice/movie/"]')[0], source = $(script).attr("src"), query = source.match(/movie\/([^\/]+)/)[1];
        Spice.add({
            id: "movie",
            name: "Movies",
            ...
            normalize: function(item) {
                var position;
                if ("Fresh" === item.ratings.critics_rating || "Certified Fresh" === item.ratings.critics_rating) {
                    position = "-256px -144px";
                } else {
                    if ("Rotten" === item.ratings.critics_rating) {
    ...
    Handlebars.registerHelper("time", function(runtime) {
        var hours = "", minutes = runtime;
        if (runtime >= 60) {
            hours = Math.floor(runtime / 60) + " hr. ", minutes = runtime % 60;
        }
